...
if __name__ == '__main__':
    ...
    # 调用保存的TF_IDF模型生成向量
    TI = TF_IDF_VECTOR()
    train_vector = TI.generate_vector(sentences=train_cut, path=settings.STATIC_DIR+'TFIDF.pkl')
    # 加载测试集
    test_file = settings.SOURCE_DATA + '1000_valid_cut.csv'
    test_top_label, test_second_label, test_cut = load_tfidf_cut(test_file)
    test_vector = TI.generate_vector(sentences=test_cut, path=settings.STATIC_DIR+'TFIDF.pkl',)
    # 尝试多层分类
